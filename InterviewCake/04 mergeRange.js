function mergeRanges(meetings) {
  ...
  returnArray = [meetings[0]];
  //
  for (let i = 0; i < meetings.length; i++) {
    let lastEl = returnArray.length - 1;
    if (meetings[i].startTime <= returnArray[lastEl].endTime) {
      returnArray[lastEl].endTime = Math.max(returnArray[lastEl].endTime, meetings[i].endTime);
    }
    else returnArray.push(meetings[i]);
  }
  return returnArray;
}
